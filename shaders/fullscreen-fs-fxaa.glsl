...
void main(void) {	
    ...
    vec2 vTextureCoord = vec2(.5)*vTexCoords + vec2(.5);

    vec2 uInvSize2 = uInvSize*vec2(1.);    //?? TODO check this? tex coords appear different to FXAA shader in other projects, so perhaps should scale here, but not obvious whether improves.

    vec4 MIDv4 = texture(uSampler, vTextureCoord);
    float MID = dot(luma,MIDv4);
    float NW = dot(luma, texture(uSampler, vTextureCoord + uInvSize2*vec2(-1.0,-1.0)));
    float NE = dot(luma, texture(uSampler, vTextureCoord + uInvSize2*vec2(1.0,-1.0)));
    float SW = dot(luma, texture(uSampler, vTextureCoord + uInvSize2*vec2(-1.0,1.0)));
    float SE = dot(luma, texture(uSampler, vTextureCoord + uInvSize2*vec2(1.0,1.0)));
