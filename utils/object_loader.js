...
function sourceDataFromObj2Or3Or5FileResponse(response, expectedVertLength, indexDataIsDiffs){
    ...
    // var sourceData = {
    //     indices: newFaces.flat(),
    //     vertices_len: expectedVertLength
    // }
    // var numVerts = newVerts.length;
    // var vertices = new Array(numVerts* (hasVertexColours? 6:3));
    // // var normals = new Array(numVerts*3);
    // if (hasVertexColours){
    //     for (var ii=0, vv=0, nn=0;ii<numVerts;ii++){
    //         var x = newVerts[ii];
    //         var p = positions[x[0]];
    //         var c = colours[x[3]];
    //         // var n = norms[x[2]];
    ...
    //     }
    // }
    // sourceData.vertices=vertices;
    // sourceData.normals= newVerts.map(x=>norms[x[2]]).flat();    //other way doesn't work!! perhaps some mesh lacks normals data. perhaps next to no point for perf anyway (maybe saves few ms)
    // if (uvs.length > 0){
    //     sourceData.uvcoords = newVerts.map(x=>uvs[x[1]]).flat();    //TODO formulate other way if see any speedup
    // }
